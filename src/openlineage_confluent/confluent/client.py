"""Confluent Cloud REST API client.

Lineage is assembled from two API surfaces:
  - Connect API   (api.confluent.cloud/connect/v1)  → connector topic mappings
  - Flink SQL API (api.confluent.cloud/sql/v1)       → statement SQL → parsed tables

Auth: HTTP Basic with a Cloud-level API key (not resource-specific).
"""

from __future__ import annotations

import json
import logging
import re
import subprocess
from typing import Any

import httpx

from openlineage_confluent.config import ConfluentConfig
from openlineage_confluent.confluent.models import (
    ConnectorInfo,
    ConnectorType,
    FlinkStatement,
    LineageEdge,
    LineageGraph,
)
from openlineage_confluent.confluent.sql_parser import parse_statement

log = logging.getLogger(__name__)

CLOUD_API = "https://api.confluent.cloud"


class ConfluentLineageClient:
    """Builds a LineageGraph from the Confluent Cloud REST API."""

    def __init__(self, cfg: ConfluentConfig, *, timeout: float = 30.0) -> None:
        self._cfg = cfg
        auth = (cfg.cloud_api_key, cfg.cloud_api_secret.get_secret_value())
        self._http = httpx.Client(
            base_url=CLOUD_API,
            auth=auth,
            timeout=timeout,
            headers={"Accept": "application/json"},
        )
        # Separate client for the regional Flink endpoint
        self._flink_http = httpx.Client(
            base_url=cfg.flink_rest_url.rstrip("/"),
            auth=auth,
            timeout=timeout,
            headers={"Accept": "application/json"},
        )

    # ------------------------------------------------------------------
    # Connectors
    # ------------------------------------------------------------------

    def list_connectors(self) -> list[ConnectorInfo]:
        """Return all managed connectors for the configured cluster."""
        env = self._cfg.environment_id
        cluster = self._cfg.cluster_id

        resp = self._http.get(
            f"/connect/v1/environments/{env}/clusters/{cluster}/connectors",
            params={"expand": "info,status"},
        )
        resp.raise_for_status()
        raw: dict[str, Any] = resp.json()

        connectors: list[ConnectorInfo] = []
        for name, data in raw.items():
            info = data.get("info", {})
            status_data = data.get("status", {})
            connector_status = status_data.get("connector", {}).get("state", "UNKNOWN")
            config = info.get("config", {})
            c_type = info.get("type", "source").lower()

            connectors.append(ConnectorInfo(
                id=f"{cluster}:{name}",
                name=name,
                status=connector_status,
                type=c_type,
                config=config,
            ))
            log.debug("Connector: %s  type=%s  status=%s", name, c_type, connector_status)
        return connectors

    # ------------------------------------------------------------------
    # Flink statements
    # ------------------------------------------------------------------

    def list_flink_statements(self) -> list[FlinkStatement]:
        """Return all Flink SQL statements via the Confluent CLI.

        The Flink REST API requires user-level OAuth tokens which are not
        available via Cloud API keys. The CLI already holds valid credentials
        and emits clean JSON.
        """
        env = self._cfg.environment_id
        try:
            result = subprocess.run(
                ["confluent", "flink", "statement", "list",
                 "--environment", env, "-o", "json"],
                capture_output=True, text=True, timeout=30,
            )
            if result.returncode != 0:
                log.warning("confluent flink statement list failed: %s", result.stderr[:300])
                return []
            raw: list[dict[str, Any]] = json.loads(result.stdout)
        except (subprocess.TimeoutExpired, json.JSONDecodeError, FileNotFoundError) as exc:
            log.warning("Failed to list Flink statements: %s", exc)
            return []

        statements: list[FlinkStatement] = []
        for item in raw:
            name   = item.get("name", "")
            sql    = item.get("statement", "")
            status = item.get("status", "UNKNOWN")
            pool   = item.get("compute_pool", "")
            if name and sql:
                statements.append(FlinkStatement(
                    name=name,
                    statement=sql,
                    status=status,
                    compute_pool=pool,
                ))
                log.debug("Flink statement: %s  status=%s", name, status)
        return statements

    # ------------------------------------------------------------------
    # Build lineage graph
    # ------------------------------------------------------------------

    def get_lineage_graph(self) -> LineageGraph:
        """Build a complete lineage graph for the cluster.

        Sources:
          - Each source connector → produces to topic(s)
          - Each sink connector   → consumes from topic(s)
          - Each Flink statement  → input tables → output tables (via SQL parsing)
        """
        env = self._cfg.environment_id
        cluster = self._cfg.cluster_id

        connectors = self.list_connectors()
        log.info("Found %d connectors", len(connectors))

        statements = self.list_flink_statements()
        log.info("Found %d Flink statements", len(statements))

        edges: list[LineageEdge] = []

        # ---- connectors ------------------------------------------------
        for conn in connectors:
            if conn.connector_type == ConnectorType.SOURCE:
                for topic in conn.topics_produced():
                    edges.append(LineageEdge(
                        source_name=conn.name,
                        source_type="external",
                        target_name=topic,
                        target_type="kafka_topic",
                        job_name=conn.name,
                        job_type="kafka_connect_source",
                        job_namespace_hint=f"kafka-connect://{env}",
                    ))
            else:  # SINK
                for topic in conn.topics_consumed():
                    edges.append(LineageEdge(
                        source_name=topic,
                        source_type="kafka_topic",
                        target_name=conn.name,
                        target_type="external",
                        job_name=conn.name,
                        job_type="kafka_connect_sink",
                        job_namespace_hint=f"kafka-connect://{env}",
                    ))

        # ---- Flink statements ------------------------------------------
        for stmt in statements:
            if not stmt.is_running():
                log.debug("Skipping non-running statement: %s (%s)", stmt.name, stmt.status)
                continue
            inputs, outputs = parse_statement(stmt.sql)
            if not inputs and not outputs:
                continue
            for inp in inputs:
                for out in outputs:
                    edges.append(LineageEdge(
                        source_name=inp,
                        source_type="kafka_topic",
                        target_name=out,
                        target_type="kafka_topic",
                        job_name=stmt.name,
                        job_type="flink_statement",
                        job_namespace_hint=f"flink://{env}",
                    ))

        log.info("Built lineage graph: %d edges from %d connectors + %d statements",
                 len(edges), len(connectors), len(statements))

        return LineageGraph(edges=edges, connectors=connectors, statements=statements)

    def close(self) -> None:
        self._http.close()
        self._flink_http.close()

    def __enter__(self) -> "ConfluentLineageClient":
        return self

    def __exit__(self, *_: object) -> None:
        self.close()
